  ...
  <div class="blog-page">
    <div class="container">
      ...
      <div class="empty-state" v-else-if="!pending">
        ...
        <h2>No articles with this tag</h2>
...
const { data: postsData, pending } = await useFetch<{ data: Post[]; meta: { hasMore: boolean } }>(
  '/api/blog/posts',
  {
    query: { tag: slug, page: page.value, limit: 9 },
  }
);
...
// Get tag name from first post or format slug
const tagName = computed(() => {
  if (posts.value.length && posts.value[0].tags) {
    const tag = posts.value[0].tags.find((t) => t.slug === slug);
...
// SEO
useSeoMeta({
  title: () => `#${tagName.value} - Blog - meskipun.win`,
  description: () => `Articles tagged with ${tagName.value}`,
});
